...
- **Картинки**: Pollinations.ai — без ключа, с автоматическим retry при 429 ошибке и fallback на стоковые фото
...
**Ошибка 429 Too Many Requests от Pollinations**
Pollinations.ai имеет лимиты на бесплатные запросы. Бот автоматически:
- Делает retry с exponential backoff (1, 2, 4 секунды)
- Использует последовательную обработку кадров (не параллельную) с задержкой 2 секунды между запросами
- При неудаче использует fallback на стоковые фото с picsum.photos

Если проблема повторяется часто:
1. Подождите 5-10 минут перед следующей генерацией
2. Уменьшите `SCENE_COUNT` в `.env` (например, с 5 до 3)
3. Используйте fallback — стоковые фото тоже выглядят неплохо
...
**Ошибка: ffmpeg died with Signals.SIGKILL: 9 (OOM killer)**
Это значит, что контейнер исчерпал память при генерации видео.

**Текущая оптимизация (уже применена):**
- Поэтапная сборка: каждый клип создаётся отдельно, потом склеивается через concat demuxer
- Разрешение 540x960 для клипов, финальный апскейл до 1080x1920
- Убран `zoompan` (очень жадный до памяти)
- Используется `ultrafast` preset и `threads=1`
- Понижен fps до 15

**Если проблема повторится:**
1. **Уменьшите `SCENE_COUNT`** в `.env` до 3 (вместо 5)
2. **Увеличьте лимит памяти** в `docker-compose.yml` до `2G` или `3G`
3. **Убедитесь, что на хосте достаточно свободной памяти** (минимум 2 ГБ)

**Требования к памяти:**
- Минимум: 1 ГБ RAM для контейнера (с SCENE_COUNT=3)
- Рекомендуется: 2 ГБ RAM (с SCENE_COUNT=5)
- Если меньше 1 ГБ — используйте SCENE_COUNT=2

**Проверка памяти:**
```bash
# Посмотрите использование памяти контейнером
docker stats shortsflow

# Посмотрите логи OOM killer (Linux)
