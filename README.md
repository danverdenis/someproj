...
- Для AI-режима с лицом: OpenCV и MediaPipe (устанавливаются через requirements.txt)
...
- **Распознавание лиц**: MediaPipe + OpenCV — извлечение лица из фото для генерации персонажа
...
Когда вы отправляете фото, бот использует MediaPipe для обнаружения лица и OpenCV для обрезки. Затем создаётся текстовое описание лица (пол, тон кожи, размер кропа), которое добавляется в промпты для генерации кадров через Pollinations. Это помогает сохранить внешность персонажа во всех кадрах видео.
...
MediaPipe требует, чтобы лицо было хорошо видно. Попробуйте:
