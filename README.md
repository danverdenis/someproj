...
Это значит, что контейнер исчерпал память при генерации видео. Решения:
1. **Увеличьте лимит памяти** в `docker-compose.yml` до `2G` (по умолчанию уже 2G)
2. **Уменьшите `SCENE_COUNT`** в `.env` до 3 (вместо 5)
3. **Убедитесь, что на хосте достаточно свободной памяти** (минимум 2 ГБ)
4. **Пересоберите образ** — в текущей версии используется оптимизированное разрешение 540x960 вместо 1080x1920

**Требования к памяти:**
- Минимум: 1.5 ГБ RAM для контейнера
- Рекомендуется: 2 ГБ RAM
- Если меньше — уменьшите `SCENE_COUNT` до 3
